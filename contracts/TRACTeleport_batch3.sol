...
contract TRACTeleport_batch3 is Ownable {
    ...
    uint256 public constant BATCH = 3;
    ...
    // EDIT from StarfleetStake: changed MIN_THRESHOLD constant from 10e25 to 1e25 = 10MM for batch 2 (changing per batch) , as per OT-RFC-12
    ...
    // EDIT from StarfleetStake: changed BOARDING_PERIOD_LENGTH constant from `30 days` to `13 days` for batch 3 (changing per batch)
    uint256 public constant BOARDING_PERIOD_LENGTH = 13 days;

    // EDIT from StarfleetStake: changed LOCK_PERIOD_LENGTH constant from `180 days` to `177 days` for batch 3 (changing per batch), as per OT-RFC-12
    uint256 public constant LOCK_PERIOD_LENGTH = 177 days;
