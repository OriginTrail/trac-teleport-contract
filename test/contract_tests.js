...
beforeEach(async () => {
	...
	stakingContract = await TRACTeleport.deployed();
	
});


contract('TRACTeleport', async function(accounts) {


	describe('Token holder deposit functionality checks ', function() {

		describe('TRACTeleport & Token basic checks', function() {
			...
			it('Cannot mistake TRAC address', async function() {
				let new_staking_contract = await TRACTeleport.new(0,"0x0000000000000000000000000000000000000000");
		...
		describe('TH1 - Token holders must be able to deposit TRAC during the boarding period', function() {
			...
			it('Cannot deposit tokens before boarding period has started', async function() {
				...
				let new_staking_contract = await TRACTeleport.new(start_time,token.address);
